...
/// duplicates. <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> is a thread-safe bag implementation, optimized for
/// scenarios where the same thread will be both producing and consuming data stored in the bag.
/// </para>
/// <para>
/// <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> accepts null reference (Nothing in Visual Basic) as a valid
/// value for reference types.
/// </para>
/// <para>
/// All public and protected members of <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> are thread-safe and may be used
...
public class ConcurrentBag<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>, ICollection, IEnumerable
{
    ...
    /// <summary>Initializes a new instance of the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> class.</summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>
    /// class that contains elements copied from the specified collection.
    /// </summary>
    /// <param name="collection">The collection whose elements are copied to the new <see
    /// cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.</param>
    ...
    /// <summary>
    /// Adds an object to the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.
    /// </summary>
    /// <param name="item">The object to be added to the
    /// <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>. The value can be a null reference
    ...
    /// <summary>
    /// Attempts to add an object to the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.
    /// </summary>
    /// <param name="item">The object to be added to the
    /// <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>. The value can be a null reference
    /// (Nothing in Visual Basic) for reference types.</param>
    ...
    /// <summary>
    /// Attempts to remove and return an object from the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.
    /// </summary>
    /// <param name="result">When this method returns, <paramref name="result"/> contains the object
    /// removed from the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> or the default value
    ...
    /// <summary>
    /// Attempts to return an object from the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> without removing it.
    /// </summary>
    /// <param name="result">When this method returns, <paramref name="result"/> contains an object from
    /// the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> or the default value of
    ...
    /// <summary>
    /// Copies the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> elements to an existing
    ...
    /// <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>. The <see
    ...
    /// cref="System.Collections.Concurrent.ConcurrentBag{T}"/> is greater than the available space from
    ...
    /// <summary>
    /// Copies the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> elements to a new array.
    /// </summary>
    /// <returns>A new array containing a snapshot of elements copied from the <see
    /// cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.</returns>
    public T[] ToArray()
    {
        if (_workStealingQueues != null)
        {
    ...
    /// <summary>
    /// Removes all values from the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.
    /// </summary>
    public void Clear()
    {
    ...
    /// cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Gets the number of elements contained in the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.
    /// </summary>
    /// <value>The number of elements contained in the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>.</value>
    /// <remarks>
    ...
    /// <summary>
    /// Gets a value that indicates whether the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> is empty.
    /// </summary>
    /// <value>true if the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/> is empty; otherwise, false.</value>
    public bool IsEmpty
    {
        get
        {
    ...
    /// with the SyncRoot; otherwise, false. For <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>, this property always
    /// returns false.</value>
    ...
    private sealed class WorkStealingQueue
    {
        ...
        /// <summary>Next queue in the <see cref="System.Collections.Concurrent.ConcurrentBag{T}"/>'s set of thread-local queues.</summary>
    ...
    // ================================================
    // The internal volatile int _currentOp; in 'WorkStealingQueue' was changed to an int
    // and all places that referenced the Operation enum we added a cast to: (int)
    // to satisfy the compiler
    
    // Internal stub to satisfy the compiler for BCL internal telemetry
    private class CDSCollectionETWBCLProvider
    {
    ...
    // Internal stub for the .NET "String Resource" manager.
    private static class SR
    {
        public const string Collection_CopyTo_TooManyElems = "The number of elements in the source is greater than the available space.";
        public const string ConcurrentCollection_SyncRoot_NotSupported = "The SyncRoot property is not supported.";
    }
